...
| Symbol| Meaning |
| --- | --- |
| PUREPASCAL | Disable all assembler implementations; Only regular Delphi Pascal code will be used.<br>Can be used if the target platform doesn’t support x86 assembler (e.g. Android). |
| OMIT_MMX | Disable the MMX assembler optimizations. |
| OMIT_SSE2 | Disable the SSE2 assembler optimizations.<br>Note that this also includes SSE, SSE4.1 and later version of SSE. |
...
![Relative performance of different optimization levels](./images/CPU-feature-performance.png)
Relative performance of different optimization levels
...
<<< @/snippets/InstructionSupport.pas
