...
The `TImage32` control contains an indexed collection of layers, referenced by the [[TCustomImage32.Layers|Layers]] property. The number of layers is limited only by the amount of free memory.
...
The basic layer’s behavior is controlled by its [[TCustomLayer.LayerOptions|LayerOptions]] property, which is a 32-bit unsigned integer value composed of [Layer Options Bits](/api/GR32_Layers/Constants/Layer%20Options%20Bits). This property allows for fast and relatively simple referencing of layers and groups of layers.
...
Using `LayerOptions` you can easily customize the appearance and modify the order in which `TImage32` repaints layers. For example, you may assign several categories to layers, using lower 24-bits of `LayerOptions` and then just change the mask in corresponding `TPaintStage` record to specify exactly which category you want to be displayed.
...
In order for a layer to be eligible to receive mouse events the layer must satisfy both of the following conditions:
  ...
  * The layer has `LOB_MOUSE_EVENTS` bit activated in its `LayerOptions`.
...
Once the layer mouse handling is done the `TImage32` control continues its normal mouse handling and generate the relevant [[TCustomImage32.OnMouseDown|OnMouseDown]], [[TCustomImage32.OnMouseMove|OnMouseMove]] or [[TCustomImage32.OnMouseUp|OnMouseUp]] event.

Note that layer mouse events can be completely disabled by setting the layer collection [[TLayerCollection.MouseEvents|MouseEvents]] property to _False_. When layer mouse events has been disabled, the `TImage32` control will generate mouse events right away, without checking for a possible receiving layer.

## Layer hit testing

As mentioned, in order for a layer to be eligible to receive mouse events, the layer must pass a _hit test_. Hit testing is performed by calling the layer’s [[TCustomLayer.HitTest|HitTest]] method:
