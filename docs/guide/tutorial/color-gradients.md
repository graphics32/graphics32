---
title: "7. Color Gradients & Fill Samplers"
---
...
begin
  ...
  try
    ...
    Points := RoundRect(FloatRect(50.0, 50.0, 550.0, 350.0), 25.0);
...
::: info
Notice that in this example we allowed `TBitmap32.SetSize` to clear the bitmap and we didn't call `TBitmap32.Clear` to set a solid background color.  
This means that the bitmap starts out fully transparent and the png we produce will therefore contain the shape we rendered with a transparent background.
:::
