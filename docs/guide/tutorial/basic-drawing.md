...
::: info
The reason why we are **not** clearing the bitmap when setting the size (`Bitmap.SetSize(Width, Height, False)`) is that on the very next line we *do* clear the bitmap to the desired color (`clWhite32` in this case). If we also cleared the bitmap in `SetSize` it would just be a waste of time.
:::
