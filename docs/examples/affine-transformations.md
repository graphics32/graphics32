---
title: "5. Affine Transformations"
---
...
This sequence of translations:
1. Moving the origin
2. Rotating about the origin
3. Moving the origin back

...is so common that it is actually built into `TAffineTransformation`. We just need to specify the center of rotation when we call the `Rotate` method and it will take care of doing it for us:
...
::: tip
Have you noticed that instead of dividing by 2 (e.g. `Width / 2`), we are multiplying by 0.5 (e.g. `Width * 0.5`)?

The reason is that division is expensive (i.e. slow) while multiplication is relatively cheap (i.e. fast). This technique, called reciprocal multiplication, is a very common optimization.
:::
---
...
begin
  ...
  try
    ...
    // The translation below is just two translations rolled into one:
    // 1. Translate(-Src.Width * 0.5, -Src.Height * 0.5)
    // 2. Translate(DstRect.Width * 0.5, DstRect.Height * 0.5)
