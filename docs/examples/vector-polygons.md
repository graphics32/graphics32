---
title: "6. Vector Polygons & Path Drawing"
---
...
::: tip
The above example draws a 5 point self-intersecting star.

In the `GR32_VectorUtils` unit you can find a versatile `Star` function that constructs non-self-intersecting stars with an arbitrary number of points: [[GR32_VectorUtils.Star|Star]].
:::
...
To draw outlines or stroked paths with custom line width, use `BuildPolyLine` in `GR32_VectorUtils.pas` to convert line segments into closed thick outline polygons, then rasterize them with `PolygonFS`.
...
var
  ...
  StrokePolygon: TArrayOfFloatPoint;
begin
  Bitmap := TBitmap32.Create;
  try
    ...
    StrokePolygon := BuildPolyLine(
      Path,                // Input contours
      False,               // Closed path = False
      12.0,                // Line thickness
      jsRound,             // Line Join: jsMiter, jsBevel, jsRound, jsSquare
      esRound              // End Cap: esButt, esSquare, esRound
    );

    // Fill thick stroke polygon onto bitmap
    PolygonFS(Bitmap, StrokePolygon, clDarkRed32);
...
3. How to stroke paths with line joins (`jsMiter`, `jsBevel`, `jsRound`, `jsSquare`) and end caps (`esButt`, `esSquare`, `esRound`).
